...
import DeleteOptionModal from "../modal/option/DeleteOptionModal";
import EditOptionModal from "../modal/option/EditOptionModal";
                  ...
                  <TableRow key={option.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <EditOptionModal />
                        <DeleteOptionModal optionId={option.id} />
                      </div>
                    </TableCell>
                  </TableRow>
                ))
        ...
        <div className="mt-6 rounded-lg border border-border bg-card p-4 flex flex-col space-y-4 md:space-y-0 items-center md:flex-row md:items-center md:justify-between">
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-medium ml-1">
              {startIndex + 1}-{Math.min(startIndex + pageSize, options.length)}
            </span>
            of
            <span className="font-medium ml-1">{options.length}</span>
          </p>
