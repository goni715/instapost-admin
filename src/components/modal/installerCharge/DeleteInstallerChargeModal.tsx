...
type TProps = {
  installerChargeId: string;
};

const DeleteInstallerChargeModal = ({ installerChargeId }: TProps) => {
  const [modalOpen, setModalOpen] = useState(false);
  const isLoading = false;

  const handleClick = () => {
    console.log(installerChargeId);
    setModalOpen(false);
  };
...
export default DeleteInstallerChargeModal;
